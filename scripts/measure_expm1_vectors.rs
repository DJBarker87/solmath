//! Dependency-free accuracy runner for retained `expm1_fixed` corpora.
//!
//! Build and run from the repository root:
//! `cargo build --release --all-features`
//! `rustc --edition=2021 scripts/measure_expm1_vectors.rs \
//!    --extern solmath=target/release/libsolmath.rlib \
//!    -L dependency=target/release/deps -o target/measure-expm1`
//! `target/measure-expm1 benchmark/prod_expm1_vectors.json \
//!    benchmark/adv_expm1_vectors.json`

use solmath::expm1_fixed;
...
fn load(path: &str) -> Vec<(i128, i128, String)> {
    ...
    while let Some((x, after_x)) = quoted_i128_after(&input, "\"x\"", cursor) {
        let Some((expected, after_expected)) = quoted_i128_after(&input, "\"expected\"", after_x) else {
            break;
        };
        let (category, after_category) = quoted_string_after(&input, "\"category\"", after_expected)
...
fn measure(path: &str) {
    ...
    let mut ordinary_errors = Vec::new();
    let mut max_error = 0u128;
    let mut max_relative_large_output_ppt = 0.0f64;
    ...
    for (x, expected, category) in rows {
        match expm1_fixed(x) {
            Ok(actual) => {
                ...
                if x.unsigned_abs() <= 2_000_000_000_000 {
                    ordinary_errors.push(error);
                }
                if expected.unsigned_abs() >= 1_000_000_000_000 {
                    max_relative_large_output_ppt = max_relative_large_output_ppt.max(
                        error as f64 / expected.unsigned_abs() as f64 * 1_000_000_000_000.0,
                    );
                }
                errors.push(error);
    ...
    if !ordinary_errors.is_empty() {
        print_stats(path, "ordinary_abs_le_2", &mut ordinary_errors, 0);
    }
    println!(
        "{{\"file\":\"{}\",\"metric\":\"max_relative_parts_per_trillion_for_outputs_ge_1\",\"value\":{:.12}}}",
        path, max_relative_large_output_ppt
    );
